#include "UAS_Confronting.h"
#include "../../../Pedestrians/NPCSpatialGrid.h"
#include "../../../Pedestrians/ConversationSubsystem.h"

void UUAS_Confronting::EnterState(APedestrian* pedestrian, UWorld* world)
{
	if (auto moveComp = pedestrian->GetComponentByClass<UPedestrianMovementComponent>())
	{
		...
		moveComp->LeaveLaneForLocation(pedestrian->GetActorLocation() + 0.9f * (pedestrian->GetConfrontTransform().GetLocation() - pedestrian->GetActorLocation()));
	}
}

void UUAS_Confronting::TickState(float deltaTime, APedestrian* pedestrian)
{
...
float UUAS_Confronting::UtilityFunction(APedestrian* pedestrian)
{
